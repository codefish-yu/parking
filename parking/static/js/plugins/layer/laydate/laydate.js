; !function (win) {
    ...
    Dates.timeVoid = function (times, index) {
        ...
        if (Dates.ymd[1] + 1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]) {
            if (index === 0 && (times < Dates.mins[3])) {
                return 1;
            } else if (index === 1 && hms[0].value == Dates.mins[3]&&times < Dates.mins[4]) {
                return 1;
            } else if (index === 2 && hms[0].value == Dates.mins[3] && hms[1].value == Dates.mins[4] && times < Dates.mins[5]) {
                return 1;
            }
    ...
    Dates.close = function () {
        ...
        //修复选择了日期还出现"不能为空"提示的问题 liangrongtong at 20181107
        if ($(Dates.elem).val().trim() != "")
            $(Dates.elem).siblings("label.error").html("");
        Dates.elem = null;
    };
