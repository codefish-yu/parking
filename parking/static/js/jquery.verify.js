...
(function ($) {	
	...
	function require(options){
		var rule={//验证规则
			...
			int: /^[0-9]*$/,
			float: /^[0-9]+\.?[0-9]*$/,
			carno: /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼领A-Z0-9]{1}[A-Z0-9]{1}[A-Z0-9]{4,5}[A-Z_0-9挂学警港澳使无]{1}$/,
		...
		this.rules= {
	        isNonEmpty: function(value, errorMsg) { //不能为空 	
	            errorMsg = errorMsg || " ";
                if (!$.trim(value).length) return errorMsg;
	        },
            ...
            isSpecial: function (value, errorMsg) {
                errorMsg = errorMsg || " ";
                if (rule.special.test(value)) return errorMsg;
            }
	    }
	};
	require.prototype={
